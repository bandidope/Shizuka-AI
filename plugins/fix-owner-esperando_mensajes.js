...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(m.chat, {text: '*[❗] La carpeta no existe o está vacía.*'}, {quoted: m});
    }
    ...
    if (filesDeleted === 0) {
      await conn.sendMessage(m.chat, {text: '*[❗] No se encontró ningún archivo para eliminar en la carpeta.*'}, {quoted: m});
    } else {
